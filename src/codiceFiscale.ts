...
export class FiscalCodeCalculator {
    ...
    private invertedMunicipalCodes: Record<
        string,
        { name: string; province: string }
    > | null = null;

    constructor() {
        // Initialize the inverted map for quick lookups
        this.buildInvertedMunicipalCodes();
    }

    /**
     * Calculate the complete fiscal code
     */
    calculateFiscalCode(person: Person): string {
        const lastNameCode = this.calculateLastNameCode(person.lastName);
        const firstNameCode = this.calculateFirstNameCode(person.firstName);
        const yearCode = this.calculateYearCode(person.birthDate);
        const monthCode = this.calculateMonthCode(person.birthDate);
        const dayGenderCode = this.calculateDayGenderCode(
            person.birthDate,
            person.gender
        );
        ...
        if (person.birthPlace) {
            municipalCode = this.getMunicipalCodeFromPlace(person.birthPlace);
        } else if (person.foreignCountry) {
            municipalCode = this.getCountryCode(person.foreignCountry);
        } else {
            throw new Error(
                'Either birthPlace or foreignCountry must be provided'
            );
        }

        // Joins the first 15 characters
        const baseCode = [
            lastNameCode,
            firstNameCode,
            yearCode,
        ...
        const checkCharacter = this.calculateCheckCharacter(baseCode);
     ...
     */
    decodeFiscalCode(fiscalCode: string): FiscalCodeData {
        if (!this.isValidFiscalCode(fiscalCode)) {
        ...
        const month = this.decodeMonth(monthCode);
        const day = this.decodeDay(dayGenderCode);
        ...
        const birthPlaceInfo = this.findBirthPlaceByCode(municipalCode);
     ...
     */
    isValidFiscalCode(fiscalCode: string): boolean {
        ...
        const calculatedCheckCharacter = this.calculateCheckCharacter(baseCode);
     ...
     */
    private calculateLastNameCode(lastName: string): string {
        // Normalize and convert to uppercase
        lastName = this.normalizeString(lastName);

        // Extract consonants
        const consonants = this.extractConsonants(lastName);
        // Extract vowels
        const vowels = this.extractVowels(lastName);
     ...
     */
    private calculateFirstNameCode(firstName: string): string {
        // Normalize and convert to uppercase
        firstName = this.normalizeString(firstName);

        // Extract consonants
        const consonants = this.extractConsonants(firstName);
        // Extract vowels
        const vowels = this.extractVowels(firstName);
     ...
     */
    private calculateYearCode(date: Date): string {
     ...
     */
    private calculateMonthCode(date: Date): string {
     ...
     */
    private calculateDayGenderCode(date: Date, gender: 'M' | 'F'): string {
     ...
     */
    private calculateCheckCharacter(code: string): string {
     ...
     */
    private extractConsonants(str: string): string {
     ...
     */
    private extractVowels(str: string): string {
     ...
     */
    private getMunicipalCodeFromPlace(place: string): string {
    ...
    private findBirthPlaceByCode(
        ...
        if (!this.invertedMunicipalCodes) {
            this.buildInvertedMunicipalCodes();
        }

        return this.invertedMunicipalCodes?.[code];
    }

    /**
     * Build the inverted municipal codes map for efficient lookups
     */
    private buildInvertedMunicipalCodes(): void {
        this.invertedMunicipalCodes = {};

        municipalities.forEach((municipality) => {
            this.invertedMunicipalCodes![municipality.code] = {
     ...
     */
    private decodeMonth(monthCode: string): number {
     ...
     */
    private decodeDay(dayCode: number): number {
